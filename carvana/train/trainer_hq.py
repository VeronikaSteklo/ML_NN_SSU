...
def train_epoch_hq(model, loader, optimizer, loss_fn, device, accum_steps=4):
    model.train()
    ...
    for i, (images, masks) in enumerate(pbar):
        ...
        summary['loss'] += loss.item() * accum_steps
        summary['iou'] += metrics['iou']
        summary['dice'] += metrics['dice']

        pbar.set_postfix(loss=loss.item() * accum_steps, iou=metrics['iou'])
...
@torch.no_grad()
def validate_epoch_hq(model, loader, loss_fn, device):
    model.eval()
    summary = {'loss': 0, 'iou': 0, 'dice': 0}
...
def run_training_hq(model, train_loader, val_loader, optimizer, loss_fn, device, epochs,
                    save_dir='outputs', patience=3, min_delta=0.01, model_name=None, accum_steps=4):
    ...
    for epoch in range(epochs):
        train_res = train_epoch_hq(model, train_loader, optimizer, loss_fn, device, accum_steps)
        val_res = validate_epoch_hq(model, val_loader, loss_fn, device)
        ...
        print(
            f"Epoch {epoch + 1:02d} | Val Loss: {val_res['loss']:.4f} | Val IoU: {val_res['iou']:.4f} | Val Dice: {val_res['dice']:.4f}")
